...
---@type { monitor: FSMonitor.Config, ui: FSMonitor.DiffConfig }
M.default_options = {
  monitor = {
    debounce_ms = 300,
...
M.options = M.default_options.monitor

---@type FSMonitor.DiffConfig
M.ui_options = M.default_options.ui

---Setup configuration with user options
---@param opts? { monitor?: FSMonitor.Config, ui?: FSMonitor.DiffConfig }
function M.setup(opts)
  opts = opts or {}

  if opts.monitor then M.options = vim.tbl_deep_extend("force", M.default_options.monitor, opts.monitor) end

  if opts.ui then M.ui_options = vim.tbl_deep_extend("force", M.default_options.ui, opts.ui) end
end

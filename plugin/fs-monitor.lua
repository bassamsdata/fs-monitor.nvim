...
vim.api.nvim_create_user_command("FSMonitor", function(opts)
  ...
  elseif subcmd == "stop" then
    ...
    if not session_id or session_id == "" then
      fs_monitor.clear_all(function()
        vim.schedule(function()
          util.notify("All sessions stopped")
        end)
      end)
    else
      fs_monitor.stop(session_id, function(changes)
        vim.schedule(function()
          util.notify(string.format("Session stopped: %s (%d changes)", session_id, #changes))
        end)
      end)
    end
  ...
  elseif subcmd == "help" then
    local help = {
      ...
      "  stop [session_id]   - Stop a session (or all if no ID)",
  ...
  complete = function(arg_lead, cmd_line, _)
    ...
    if num_args == 1 or (num_args == 2 and not cmd_line:match("%s$")) then
      local subcommands = { "start", "stop", "diff", "show", "stats", "help" }
    ...
    if num_args >= 2 then
      local subcmd = args[2]
      if subcmd == "stop" or subcmd == "diff" or subcmd == "show" or subcmd == "stats" then
